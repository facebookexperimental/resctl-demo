...
impl<'a> Grapher<'a> {
    ...
    fn setup_view(sel: &DataSel, series: &DataSeries) -> (ContinuousView, f64, f64) {
        ...
        xlabel += &if lines.low.0 > 0.0 {
            ...
            format!("min={:.3} ", lines.low.1 * yscale)
        };
        xlabel += &if lines.high.0 < vrate_max {
            format!(
                "high-infl=({:.1}, {:.3}) ",
                lines.high.0,
                lines.high.1 * yscale
            ...
            format!("max={:.3} ", lines.high.1 * yscale)
        };
        xlabel += &format!("error={:.3}", series.error * yscale);
    ...
    fn plot_one_text(&mut self, sel: &DataSel, series: &DataSeries) -> Result<()> {
        const DIMENSION: (u32, u32) = (80, 24);

        let (view, vrate_max, yscale) = Self::setup_view(sel, series);

        let mut lines = vec![];
        for i in 0..DIMENSION.0 {
            let vrate = vrate_max / DIMENSION.0 as f64 * i as f64;
        ...
        let page = Page::single(&view).dimensions(DIMENSION.0, DIMENSION.1);
