...
impl Hashd {
    ...
    fn report(&mut self) -> Result<HashdReport> {
        ...
        Ok(HashdReport {
            svc: svc_r,
            load: hashd_r.hasher.rps / self.rps_max as f64,
...
impl HashdSet {
    ...
    pub fn report(&mut self) -> Result<[HashdReport; 2]> {
        Ok([self.hashd[0].report()?, self.hashd[1].report()?])
    }
}
