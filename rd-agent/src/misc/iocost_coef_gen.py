...
parser.add_argument(
    "--testfile-dev", metavar="DEV", help="Override testfile device detection"
)
parser.add_argument(
    "--testfile-size-gb",
    type=float,
...
parser.add_argument(
    "--json", metavar="FILE", help="Store the results to the specified json file"
)
parser.add_argument(
    "--model-override",
    metavar='"rbps=XXX rseqiops=XXX..."',
    help="Use the specified model params instead of testing for them",
)
parser.add_argument(
    "--qos-override",
    metavar='"rpct=XXX rlat=XXX wpct=XXX wlat=XXX min=XXX max=XXX"',
...
def dir_to_dev(path):
    ...
    devname = subprocess.run(
        shlex.split(f"findmnt -nvo SOURCE -T{path}"), stdout=subprocess.PIPE
    ).stdout
    ...
    if devname.startswith("/dev/dm") or devname.startswith("/dev/md"):
        info(
            f"{devname} is composite, you probably want to use --testfile-dev to specify the underlying device"
        )
...
def create_testfile(path, size):
    ...
    subprocess.check_call(shlex.split(f"rm -f {path}"))
    subprocess.check_call(shlex.split(f"touch {path}"))
    subprocess.call(shlex.split(f"chattr +C {path}"))
    cmd = shlex.split(
        f"dd if=/dev/urandom of={path} count={size} "
...
def run_fio(testfile, duration, iotype, iodepth, blocksize, jobs, rate_iops, outfile):
    ...
    cmd = shlex.split(
        f"fio --direct=1 --ioengine=libaio --name=coef "
...
if args.model_override is None:
    (rbps, rseqiops, rrandiops, wbps, wseqiops, wrandiops) = determine_model(
        args, testfile
    )
else:
...
if args.qos_override is None:
    (rpct, rlat, wpct, wlat, vmin, vmax) = determine_qos(
        args, testfile, rrandiops, wrandiops
    )
else:
...
info("")
print(
    f"io.cost.model: {devnr} rbps={rbps} rseqiops={rseqiops} "
    f"rrandiops={rrandiops} wbps={wbps} wseqiops={wseqiops} "
    f"wrandiops={wrandiops}"
)
print(
    f"io.cost.qos: {devnr} rpct={rpct} rlat={rlat} "
