...
* Tail latency matters but a single miss isn't gonna ruin everything.

* Should hit a reasonable balance between throughput and latency.
...
* Have combination of page cache and anonymous memory access pattern with
  hot and cold areas like any other application.

* Some write IOs for logs and other persistent data.

While some benchmarks can measure different aspects of system performance,
it is challenging to gauge what the aggregate behavior would be for an
application like above. While nothing is more accurate than using production
workloads directly, that usually is a cumbersome and challenging process.
...
We all want our workloads to utilize the machines as much as possible and
spend a considerable amount of time and effort to tune the workload so that,
under the full load, it saturates the machine just enough so that all the
available resources are utilized while leaving sufficient buffer to keep the
machine from falling apart when the maintenance cron job kicks in at
midnight.

To ease testing, rd-hashd can benchmark and size itself to automatically
determine the followings so that it can saturate all local resources.
...
* The max RPS the machine can serve - the RPS at which all CPUs are fully
  utilized without any memory or IO contentions.
...
the latency target while servicing more reclaims and refaults and thus the
...
with different IO devices. You effectively gain more usable memory with
...
and parameters which can be applied immedately while rd-hashd is running.

Once benchmark is complete, the arguments are stored in
...
* file_max_frac: Maximum fraction of page cache out of memory footprint. 0.0
  means all of memory foot print is anonymous memory, 1.0 all page cache.
  The actual page cache fraction is determined by the `file_frac` parameter
  which is capped by this argument. Defaults to 0.25.

For full explanation, see `rd-hashd --help`.
...
The following parameters are determined by benchmark. Don't change manually.
...
* mem_frac: Memory footprint scaling factor. rd-hashd will use
  `file_max_frac` * `mem_frac` bytes. This can be changed through demo
  interface.
...
by default, while running the benchmark as the system should be able to
...
rd-hashd and other applications once benchmark is complete.
...
reserve size further to see drastic difference on high performance SSDs.
This can be offset with making the memory access pattern flatter by
increasing `file_addr_stdev_ratio` and `anon_addr_stdev_ratio` in the
`params.json` file.
