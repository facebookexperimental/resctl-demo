...
hashd's sensitivity to the steady availability of CPU, memory, and IO, makes
it an honest indicator of how well resource isolation is working on all
fronts on the host.

Let's fire up hashd to get rolling:

%% (                             : [ Start hashd at 70% load ]
%% knob hashd-load 0.7
%% on hashd
%% )

Watch the panel to your left to see the RPS ramping up. You can check the
logs for warnings and errors as well.
...
launch a compile job. Because it's not interactive, and therefore not bound
by user input, it will eat up as many CPU cycles, as much IO bandwidth, and
as much memory for caches as it can get its hands on. It's doing useful
...
See the graph for the steep drop in RPS for hashd: That's the compile job
...
Watch the stable RPS while the compile job still makes forward progress:
